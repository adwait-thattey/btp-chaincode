...
func (s *SmartContract) AddData(ctx contractapi.TransactionContextInterface) error {
	...
	devicePrivateDetailsJSONasBytes, err := json.Marshal(devicePrivateDetails)
	if err != nil {
		return fmt.Errorf(err.Error())
	}

	// try to put on o1
	err = ctx.GetStub().PutPrivateData(o1, dataInput.Timestamp, devicePrivateDetailsJSONasBytes)
	if err != nil {
		fmt.Println("failed to put data private details: %s", err.Error())
	}

	// try to put in o2
	err = ctx.GetStub().PutPrivateData(o2, dataInput.Timestamp, devicePrivateDetailsJSONasBytes)
	if err != nil {
		fmt.Println("failed to put data private details: %s", err.Error())
	}
	...
	if isSharingAsBytes != nil && string(isSharingAsBytes) == "true" {
		err = ctx.GetStub().PutPrivateData(o1o2_dataSharing, dataInput.Timestamp, devicePrivateDetailsJSONasBytes)
		if err != nil {
